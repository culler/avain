 ...
 *    + a symlink MacOS/lib to the lib directory in the Python framework
 *      (This is needed to allow tkinter to find init.tcl)
 *    + a python script Resources/main.py 
 *      (This program runs the script as the main executable,
 *      using MacOS/Python.)  
 */

static void debugMessage(char *msg) {
    FILE *logfile = fopen("/tmp/avain.log", "a");
    fprintf(logfile, "%s\n", msg);
    fclose(logfile);
}
...
int main(int argc, char **argv, char **envp) {
    ...
    CFURLRef URL;
    CFStringRef string;
    /* debugMessage("Starting Avain"); */
    URL = CFBundleCopyExecutableURL(bundle);
    string = CFURLCopyFileSystemPath(URL, kCFURLPOSIXPathStyle);
    CFRelease(URL);
    CFStringGetCString(string, executablePath, PATH_MAX,
